...
import { CelService } from './shared/cel.service';
import { CelDataService } from './shared/cel-data.service';
...
@Component({
  selector: 'app-teclider',
  templateUrl: './teclider.page.html',
  styleUrls: ['./teclider.page.scss']
})
export class TecliderPage implements OnInit {
  cels:any;
  cloudFiles = [];

  constructor(private celService: CelService, 
  ...
  ngOnInit() {
    this.loadFiles();
    this.cels = this.celService.getAll();
  ...
  delete(cel: any) {
    this.alert.showConfirmDelete(cel.name,() => this.executeRemove(cel));
  }
  private executeRemove(cel: string) {
    this.celService.delete(cel);
      try {          
